...
def main():
    ...
    with col2:
        ...
        if st.button('📊'):
            with get_openai_callback() as cb:
                st.write(sdf.chat("Show me a graph with monthly total of the column \"outcome\", grouped by the column \"category\". Exclude rows where categories is: T10, S.I Systems, Policy Reporter, Transfers and CRA. Replace empty (NaN) values with 0."))
                st.write(cb)
